...
export default function FleetJobsPage() {
  ...
  const createWorkshopJob = async () => {
    ...
    try {
      ...
      const job_id = "Workshop-" + year + "-" + Math.floor(Math.random() * 1000).toString().padStart(3, "0");
      ...
      const selectedWorkshop = workshops.find(w => String(w.id) === String(createJobForm.selected_workshop_id))
  ...
  const handleDelete = async (job: WorkshopJob) => {
    const confirmed = window.confirm(`Are you sure you want to delete job for vehicle ${job.registration_no}?`);
    if (!confirmed) return;
  ...
  useEffect(() => {
    ...
    const noRecentJobWorkshop = availableWorkshops.find(w => !(lastAssigned[w.id]));
