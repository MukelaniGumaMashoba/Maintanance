...
import { FileText, Eye, CheckCircle, XCircle, ArrowLeft, Truck, MapPin, Phone, User, Calendar, DollarSign, Wrench, Clock, AlertTriangle } from "lucide-react"
        ...
        const fetchJobAndVehicle = async () => {
            ...
            if (jobData.technician_id) {
                const { data: techData, error: techError } = await supabase
                    .from("technicians")
                    .select("*")
                    .eq("id", Number(jobData.technician_id))
                    .single()
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="bg-white shadow-sm border-b px-6 py-4">
                <div className="flex items-center justify-between">
                    ...
                    <h1 className="text-xl font-bold text-black">Klava Plant Hire - Job Details</h1>
                </div>
            </div>
                    ...
                    <CardHeader className="bg-orange-500 text-white">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                            <div>
                                ...
                                <p className="text-orange-100 capitalize">{job.job_type} Service</p>
                            </div>
                            <div className="text-right">
                                <Badge className={`${getStatusColor(job.status)} px-3 py-1`}>
                                    {job.status}
                                </Badge>
                                <p className="text-orange-100 text-sm mt-1">{new Date(job.created_at).toLocaleDateString()}</p>
                            </div>
                        </div>
                    </CardHeader>
                </Card>
                        ...
                        <CardContent className="p-6">
                            <div className="space-y-4">
                                <div className="bg-gray-50 p-3 rounded">
                                    ...
                                    <p className="font-semibold">{job.client_name || "Not specified"}</p>
                                </div>
                                <div className="bg-gray-50 p-3 rounded">
                                    <p className="text-sm text-gray-600">Phone Number</p>
                                    <p className="font-semibold">{job.client_phone || "Not provided"}</p>
                                </div>
                                <div className="bg-gray-50 p-3 rounded">
                        ...
                        <CardContent className="p-6">
                            {technician ? (
                                ...
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-sm text-gray-500">Technician will be assigned after approval</p>
                                </div>
                            )}
                        </CardContent>
                    </Card>
                            ...
                            <div className="space-y-4 mb-6">
                                <div className="bg-green-50 p-3 rounded border border-green-200">
                                    ...
                                    <p className="text-xl font-bold text-green-800">
                                        {job.estimated_cost ? `R ${job.estimated_cost.toFixed(2)}` : "TBD"}
                                    </p>
                                </div>
                                <div className="bg-blue-50 p-3 rounded border border-blue-200">
                                    <p className="text-sm text-blue-700">Actual Cost</p>
                                    <p className="text-xl font-bold text-blue-800">
                                        {job.actual_cost ? `R ${job.actual_cost.toFixed(2)}` : "Pending"}
                                    </p>
                                </div>
                            </div>
