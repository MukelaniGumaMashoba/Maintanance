...
export default function StockLevelsPage() {
  ...
  const totalValue = filteredParts.reduce((sum, part) => 
    sum + (parseInt(part.quantity || "0") * (part.price || 0)), 0
  )

  const lowStockCount = parts.filter(p => parseInt(p.quantity || "0") <= 5).length
  const outOfStockCount = parts.filter(p => parseInt(p.quantity || "0") === 0).length

  const exportToCSV = () => {
    const headers = ["Item Code", "Description", "Category", "Quantity", "Price", "Total Value", "Status"]
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64 text-gray-500">
        <Package className="animate-pulse mr-2 h-6 w-6" /> Loading stock levels...
      </div>
    )
  }
                  ...
                  return (
                    <tr key={part.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3 text-center">
                        <span className={`font-medium ${quantity <= 5 ? "text-red-600" : ""}`}>
                          {quantity}
                        </span>
                      </td>
                      <td className="p-3 text-right">R{(part.price || 0).toFixed(2)}</td>
                      <td className="p-3 text-right font-medium">R{totalValue.toFixed(2)}</td>
                      <td className="p-3 text-center">
